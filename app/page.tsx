...
const fallback: Project[] = [
  {id:'tnabbah',title:'TNABBAH',subtitle:'Smart Vehicle Diagnostics',description:'Cross-platform vehicle diagnostics system using ELM327 BLE OBD-II, live ECU telemetry, DTC analysis, MQTT, maintenance workflows and multi-vehicle data.',tags:['React Native','FastAPI','MQTT','PostgreSQL','BLE'],image_url:'/projects/tnabbah.jpg',live_url:'https://tnabbah.vercel.app/',featured:true,sort_order:1},
  {id:'madad',title:'MADAD',subtitle:'Field Resource & Operations Orchestration Platform',description:'Field-operations platform for incident lifecycle management, team and resource allocation, optimization-based dispatch, RBAC, realtime tracking and SLA monitoring.',tags:['Next.js','Express','Prisma','PostgreSQL','Realtime'],image_url:'/projects/madad.jpg',live_url:'https://madad-hajj-field-resource-operation.vercel.app',featured:true,sort_order:2},
  {id:'switchboard',title:'SWITCHBOARD',subtitle:'Visual IT Automation Builder',description:'Visual workflow builder for IT operations with an execution-engine architecture covering HTTP, databases, PowerShell, SSH, conditions, approvals and scheduled triggers.',tags:['Next.js','Node.js','PostgreSQL','Docker','Automation'],image_url:'/projects/switchboard.jpg',featured:true,sort_order:3},
...
function ProjectVisual({project}:{project:Project}){
  ...
  return <div className="posterVisual">
    {project.image_url && !failed ? <img src={project.image_url} alt={`${project.title} interface`} onError={()=>setFailed(true)}/> : <><div className="fakeWindow"><i/><i/><i/></div><span>PROJECT PREVIEW</span></>}
  </div>;
}
...
function PortraitVisual(){
  ...
  return <div className="portrait placeholder">
    {!failed ? <img src="/images/profile.jpg" alt="Ghala Al-Hashmi Al-Ameer" onError={()=>setFailed(true)}/> : <span>PROFILE PHOTO</span>}
  </div>;
}
...
export default function Home(){
  ...
  return <main>
    <nav>
      ...
      <div className="navlinks"><a href="#about">PROFILE</a><a href="#hub">PROJECTS</a><a href="#proof">ENGINEERING</a><a href="#experience">EXPERIENCE</a><a href="#contact">CONTACT</a></div>
    ...
    <section id="top" className="hero">
      ...
      <div className="topline"><span>GHALA AL-HASHMI AL-AMEER</span><span>SOFTWARE ENGINEERING · 2026</span></div>
      ...
      <div className="heroCollage">
        <motion.div className="introCard stickerCard" initial={{rotate:-3,opacity:0}} animate={{rotate:-1,opacity:1}}><span className="tape tapeA"/><p className="eyebrow">SOFTWARE ENGINEER</p><h1>GHALA<br/><i>AL-HASHMI</i></h1><p className="introText">B.Sc. Software Engineering graduate focused on backend development, full-stack systems and IT automation.</p></motion.div>
        <motion.div className="portraitFrame" initial={{scale:.96,opacity:0}} animate={{scale:1,opacity:1}}><PortraitVisual/><div className="photoTag">MAKKAH · JEDDAH</div></motion.div>
        <div className="heroSide"><div className="miniCard"><b>CORE AREAS</b><small>BACKEND · APIs · DATABASES · INTEGRATIONS</small></div><div className="yearBadge"><span>GRAD.</span><strong>2026</strong><span>UQU</span></div></div>
    ...
    <section id="proof" className="experienceSection paperTexture">
      ...
      </div></div><div className="toolPoster"><span className="tape tapeB"/><p className="eyebrow">TECHNICAL SCOPE</p><div className="proofIcons"><ServerCog/><Database/><Workflow/></div><div className="toolCloud"><span>CLIENT</span><span>REST API</span><span>AUTH</span><span>BUSINESS LOGIC</span><span>POSTGRESQL</span><span>REALTIME</span><span>DEPLOYMENT</span></div></div></div>
    </section>
