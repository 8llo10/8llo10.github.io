...
import { Braces, Database, Server, Network } from "lucide-react";
...
function Reveal({ children, className="", id }: {children:React.ReactNode; className?:string; id?:string}) {
  return <motion.section id={id} className={className} variants={reveal} initial="hidden" whileInView="show" viewport={{once:true, amount:.1}} transition={{duration:.62, ease}}>{children}</motion.section>;
}
...
function Brand({settings}:{settings:SiteSettings}){
  return <a href="#home" className="brand-lockup" aria-label="Home">
    {settings.logo_url ? <img src={settings.logo_url} alt="Ghala logo"/> : <span className="logo-slot">LOGO</span>}
    <span className="brand-word">GHALA</span>
  </a>;
}

function Hero({ section, settings }: { section: Section; settings: SiteSettings }) {
  return <section className="editorial-hero" id="home">
    <div className="hero-kicker"><span>SOFTWARE ENGINEER</span><span>PORTFOLIO · 2026</span></div>
    <motion.div className="hero-display" initial={{opacity:0, y:34}} animate={{opacity:1,y:0}} transition={{duration:.8,ease}}>GHALA</motion.div>
    ...
    <motion.div className="hero-portrait" initial={{opacity:0,scale:.97,y:22}} animate={{opacity:1,scale:1,y:0}} transition={{duration:.85,delay:.08,ease}}>
      {section.image_url ? <img src={section.image_url} alt="Ghala portrait"/> : <Placeholder hint="YOUR PORTRAIT GOES HERE · transparent PNG preferred · 4:5"/>}
    </motion.div>
    ...
    <div className="hero-socials"><External href={settings.github_url}><Github size={17}/></External><External href={settings.linkedin_url}><Linkedin size={17}/></External><External href={settings.email.startsWith("YOUR_")?"":`mailto:${settings.email}`}><Mail size={17}/></External></div>
...
function Projects({section,projects}:{section:Section;projects:Project[]}) {
  ...
  return <Reveal id="projects" className="portfolio-panel projects-panel">
    ...
    <div className="project-editorial-grid">{sorted.map((p,i)=><motion.article className="editorial-project" key={p.slug} whileHover={{y:-5}} transition={{duration:.22}}>
      <div className="project-meta"><span>{String(i+1).padStart(2,"0")}</span><div><strong>{p.title}</strong><small>{p.subtitle}</small></div><External href={p.project_url||p.github_url||""} className="project-arrow"><ArrowUpRight size={17}/></External></div>
      <div className="project-image">{p.image_url?<img src={p.image_url} alt={p.title}/>:<Placeholder hint={`PROJECT IMAGE · ${p.title} · upload from /admin · 16:10`} ratio="16 / 10"/>}</div>
...
function About({section}:{section:Section}) { return <Reveal id="about" className="portfolio-panel about-panel">
  ...
  <div className="about-photo">{section.image_url?<img src={section.image_url} alt="About Ghala"/>:<Placeholder hint="SECONDARY PHOTO · event / work / candid · 4:3" ratio="4 / 3"/>}</div>
...
function Custom({section}:{section:Section}) {
 const pos=String(section.config?.imagePosition||"right");
 const tone=String(section.config?.tone||"plain");
 return <Reveal id={section.key} className={`portfolio-panel custom-panel custom-${section.layout} image-${pos} tone-${tone}`}>
   <div className="custom-copy"><span className="panel-index">{section.eyebrow||"CUSTOM"}</span><h2>{section.title}</h2><p>{section.body}</p></div>
   <div className="custom-image">{section.image_url?<img src={section.image_url} alt={section.title}/>:<Placeholder hint={`SECTION IMAGE · ${section.title} · upload from /admin`} ratio="16 / 10"/>}</div>
 </Reveal>
}
...
export default function PortfolioClient({sections,projects,settings}:{sections:Section[];projects:Project[];settings:SiteSettings}) {
 ...
 return <main className="portfolio-page">
   <SplashLoader/>
   <nav className="editorial-nav"><Brand settings={settings}/><div className="nav-center"><a href="#projects">Work</a><a href="#about">About</a><a href="#experience">Experience</a><a href="#skills">Stack</a></div><div className="nav-end"><ThemeToggle/><a href="#contact" className="nav-contact">CONTACT <ArrowUpRight size={14}/></a></div></nav>
   <div className="portfolio-frame">
    ...
    <footer className="editorial-footer"><span>© {new Date().getFullYear()} {settings.name}</span><span>BUILD · SOLVE · SHIP</span></footer>
   </div>
 </main>;
}
